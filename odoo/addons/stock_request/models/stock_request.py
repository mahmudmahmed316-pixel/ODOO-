...
class StockRequest(models.Model):
    ...
    def _compute_qty(self):
        for request in self:
            ...
            request.quantity = uom._compute_quantity(done_qty, request.product_uom_id)
    ...
    def check_done(self):
        ...
        for request in self:
            ...
            quantity = request.product_id.uom_id._compute_quantity(
                allocated_qty, request.product_uom_id
            )
            if (
                float_compare(
                    quantity, request.product_uom_qty, precision_digits=precision
                )
                >= 0
            ):
                request.action_done()
            elif request._check_cancel_allocation():
                # If quantity=0 and qty_cancelled>0 it's cancelled
                request.write({"state": "cancel"})
        return True
